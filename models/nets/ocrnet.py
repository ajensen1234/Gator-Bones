...
class SpatialOCRNet(nn.Module):
    ...
    Best performance is usually: HRNet + OCRNet + Segfix
    """
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.num_classes = 2 # FIXME: Get from config
        self.backbone = BackboneSelector(config).get_backbone()

        # extra added layers
        if "wide_resnet38" in self.config.get("network", "backbone"):
            in_channels = [2048, 4096]
        else:
            in_channels = [1024, 2048]
        self.conv_3x3 = nn.Sequential(
            nn.Conv2d(in_channels[1], 512, kernel_size=3, stride=1, padding=1),
            ModuleHelper.BNReLU(512, bn_type=self.config.get("network", "bn_type")),
        )
        ...
        self.spatial_ocr_head = SpatialOCR_Module(
            ...
            scale=1,
            dropout=0.05,
            bn_type=self.config.get("network", "bn_type"), #FIXME
        )
        ...
        self.dsn_head = nn.Sequential(
            ...
            ModuleHelper.BNReLU(512, bn_type=self.config.get("network", "bn_type")), #FIXME: bn_type
            nn.Dropout2d(0.05),
            nn.Conv2d(
                512, self.num_classes, kernel_size=1, stride=1, padding=0, bias=True
            ),
